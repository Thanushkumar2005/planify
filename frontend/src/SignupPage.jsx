...
function SignupPage({ onSwitch, onLoginSuccess }) {
  ...
  const handleSubmit = async (event) => {
    ...
    try {
      ...
      setStatusMessage(detail ? `${message} (${detail})` : message);
      setStatusType("error");
    } finally {
    ...
    <div className="min-h-screen bg-gradient-to-br from-slate-950 via-slate-900 to-slate-950 flex items-center justify-center px-4 py-10">
      <div className="w-full max-w-md rounded-3xl bg-gradient-to-br from-sky-900/95 via-indigo-950/90 to-fuchsia-900/95 border border-slate-700 shadow-2xl shadow-slate-950/40 backdrop-blur-lg p-8">
        <div className="mb-8 text-center">
          <h1 className="text-4xl font-bold tracking-tight text-sky-200">Join Planify</h1>
          <p className="mt-3 text-sm text-slate-200">
            Create your account and start organizing tasks in style.
          </p>
        </div>
        ...
        <form onSubmit={handleSubmit} className="space-y-6">
          ...
          <button
            ...
            className="w-full rounded-2xl bg-fuchsia-500 px-4 py-3 text-sm font-semibold text-slate-950 transition hover:bg-fuchsia-400 focus:outline-none focus:ring-2 focus:ring-fuchsia-400 focus:ring-offset-2 focus:ring-offset-slate-950 disabled:cursor-not-allowed disabled:opacity-70"
          >
